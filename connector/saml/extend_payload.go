...
func (c *provider) ExtendPayload(scopes []string, payload []byte, cdata []byte) ([]byte, error) {
	// var cd ConnectorData
	// originalClaims := map[string]interface{}{}
	var originalClaims map[string]interface{}

	c.logger.Info("(saml) ExtendPayload scopes", scopes)
	c.logger.Info("(saml) ExtendPayload payload", string(payload))
	c.logger.Info("(saml) ExtendPayload cdata", string(cdata))
